...
import {
  ...
  Repeat, Trash2, Image as ImageIcon
} from 'lucide-react';
...
export default function ProfilAnaIcerik({
  ...
  // Tablar: gonderiler, makaleler, hakkinda, gecmis, kaydedilenler, koleksiyonlar
  const [activeTab, setActiveTab] = useState<string>('gonderiler');
  ...
  // Gönderi ve Yeniden Paylaşım (Repost) Eyaletleri
  const [postsList, setPostsList] = useState<any[]>(gonderiler);
  const [repostsList, setRepostsList] = useState<any[]>(reposts);
  ...
  useEffect(() => {
    setRepostsList(reposts);
  }, [reposts]);

  // İlgi Alanları/Etiketler Eyaletleri
  const [userTopics, setUserTopics] = useState<any[]>(konular);
  const [newTopicName, setNewTopicName] = useState('');
  const [isAddingTopic, setIsAddingTopic] = useState(false);

  useEffect(() => {
    setUserTopics(konular);
  }, [konular]);
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRepostsList((prev) => prev.filter((r) => r.id !== postId));
      } else {
  ...
  // İlgi alanı / etiket ekleme
  const handleAddTopic = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTopicName.trim() || !firebaseUser) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || 'Etiket eklenemedi.');
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  // İlgi alanı / etiket silme
  const handleRemoveTopic = async (topicId: number) => {
    if (!firebaseUser) return;
    try {
      const res = await fetch(`/api/user/topics?topicId=${topicId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${firebaseUser.idToken}` }
      });
  ...
  const tabs = [
    ...
    { id: 'paylasilanlar', label: 'Yeniden Paylaşılanlar', count: reposts.length },
    { id: 'hakkinda', label: 'Hakkında' },
    { id: 'gecmis', label: 'Mesaj Geçmişi' },
    ...(isOwnProfile ? [{ id: 'kaydedilenler', label: 'Kaydedilenler', count: savedItems.length }] : []),
      ...
      <div className="relative bg-card border border-border rounded-2xl p-6 sm:p-8 mb-8 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10">
          ...
          <div className="flex-1 min-w-0 w-full text-center md:text-left flex flex-col gap-4">
            ...
            {/* 1. Satır: Kullanıcı Adı, Link ve Butonlar */}
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-center md:justify-start">
              <div className="flex flex-col items-center md:items-start">
                <h1 className="text-xl font-bold text-foreground">@{profil.username}</h1>
                <a 
                  href={`/${profil.username}`}
                  className="text-xs text-muted-foreground hover:text-erciyes-red transition-colors font-mono mt-0.5"
                >
                  zirvekampus.com/{profil.username}
                </a>
              </div>
            ...
            <div className="flex flex-col gap-1 text-left">
              ...
              <div className="mt-3 border-t border-border/40 pt-3 space-y-2">
                <span className="block text-[9px] font-extrabold uppercase tracking-widest text-muted-foreground">İlgi Alanları / Etiketler</span>
                <div className="flex flex-wrap gap-1.5 items-center">
                  {userTopics.map((topic) => (
                    <span
                      key={topic.id}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-secondary/60 text-muted-foreground border border-border"
                    >
                      #{topic.name}
                      {isOwnProfile && (
                        <button
                  ...
                  {isOwnProfile && (
                    <form onSubmit={handleAddTopic} className="inline-flex items-center gap-1">
                      <input
                        type="text"
                        value={newTopicName}
                        onChange={(e) => setNewTopicName(e.target.value)}
                        placeholder="Yeni etiket..."
                        className="px-2.5 py-0.5 rounded-full text-[10px] bg-background text-foreground placeholder:text-muted-foreground border border-border focus:outline-none focus:border-erciyes-red w-24 transition-all"
                        maxLength={25}
                      ...
                      <button
                        ...
                        className="p-1 rounded-full bg-secondary hover:bg-secondary-hover border border-border text-muted-foreground hover:text-foreground transition-all disabled:opacity-50"
                      >
      ...
      <div className="space-y-4">
        ...
        {activeTab === 'gonderiler' && (
          <div className="space-y-6">
            {/* Gönderi Paylaşma Arayüzü (Sadece Profil Sahibi) */}
            {isOwnProfile && (
              <PostCreateForm
                inline
                placeholder="Kampüste neler oluyor? Bir gönderi paylaş..."
                onSuccess={(newPost) => {
            ...
            {postsList.length === 0 ? (
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p className="font-medium">Henüz paylaşılmış bir gönderi yok</p>
              </div>
            ) : (
              <div className="space-y-4">
                {postsList.map((post) => {
                  ...
                  return (
                    <article
                      key={post.id}
                      className="bg-card border border-border rounded-2xl overflow-hidden hover:border-border/80 transition-all duration-300 shadow-sm p-5 space-y-4"
                    >
                      {/* Üst Yazar Bilgisi */}
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {(isOwnProfile || dbUser?.role === 'admin' || dbUser?.role === 'editor') && (
                          <button
                      ...
                      {/* Gönderi İçeriği */}
                      <p className="text-sm text-muted-foreground leading-relaxed break-words font-sans">
                        {post.description}
                      </p>
                      ...
                      {post.image_urls && post.image_urls.length > 0 && (
                        <div className={cn(
                          "w-full overflow-hidden bg-black rounded-2xl border border-border/50",
                      ...
                      {/* Alt Etkileşimler */}
                      <div className="flex items-center gap-6 text-xs text-muted-foreground border-t border-border/40 pt-3">
                        <button
                        ...
                        <button
                          ...
                          className={cn(
                            'flex items-center gap-1.5 font-bold transition-colors hover:text-erciyes-gold',
        ...
        {/* TAB: Yeniden Paylaşılanlar */}
        {activeTab === 'paylasilanlar' && (
          <div className="space-y-6">
            {repostsList.length === 0 ? (
              <div className="text-center py-16 text-muted-foreground">
                <Repeat className="w-10 h-10 mx-auto mb-3 opacity-30" />
                <p className="font-medium">Henüz yeniden paylaşılan bir gönderi yok</p>
              </div>
            ) : (
              <div className="space-y-4">
                {repostsList.map((item) => {
                  const isArticle = item.oge_tipi === 'article';
                  const isForum = item.oge_tipi === 'forum';
                  const isProject = item.oge_tipi === 'project';
                  ...
                  if (isArticle) {
                    authorName = item.author?.display_name ?? item.author?.username ?? 'Yazar';
                    avatarUrl = item.author?.avatar_url ?? '';
                    title = item.title;
                    url = `/makale/${item.slug}`;
                    desc = item.excerpt ?? '';
                  } else if (isForum) {
                    authorName = item.creator?.display_name ?? item.creator?.username ?? 'Yazar';
                    title = item.title;
                    url = `/forum/${item.slug}`;
                    desc = 'Forum Başlığı';
                  } else if (isProject) {
                    ...
                    avatarUrl = item.author?.avatar_url ?? '';
                    title = item.title;
                    url = item.fakulte === 'gonderi' ? `/${item.author?.username}` : `/projeler/${item.id}`;
                    desc = item.description ?? '';
                  }

                  return (
                    <article
                      key={`${item.oge_tipi}-${item.id}`}
                      className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-3 relative overflow-hidden"
                    >
                      <div className="absolute top-0 left-0 right-0 h-0.5 bg-erciyes-gold" />
                      
                      {/* Repost Header */}
                      <div className="flex items-center gap-1.5 text-xs text-erciyes-gold font-semibold mb-1">
                        <Repeat className="w-3.5 h-3.5" />
                        <span>{displayName} Yeniden Paylaştı</span>
                      </div>

                      {/* Item content card */}
                      <div className="p-4 rounded-xl bg-secondary/20 border border-border/40 hover:border-border transition-all">
                        <div className="flex items-center gap-2 mb-2">
                          {avatarUrl ? (
                            <Image src={avatarUrl} alt={authorName} width={24} height={24} className="rounded-full" />
                          ) : (
                            <div className="w-6 h-6 rounded-full bg-secondary flex items-center justify-center font-bold text-muted-foreground text-[10px]">
                              {authorName.slice(0, 2).toUpperCase()}
                            </div>
                          )}
                          <span className="font-bold text-xs">@{authorName}</span>
                          <span className="text-xs text-muted-foreground">•</span>
                          <span className="text-[10px] uppercase font-bold text-erciyes-red">
                            {isArticle ? 'Makale' : isForum ? 'Forum Konusu' : item.fakulte === 'gonderi' ? 'Gönderi' : 'Proje'}
                          </span>
                        </div>

                        <Link href={url}>
                          <h4 className="font-semibold text-sm hover:text-erciyes-red transition-colors mb-1">
                            {title}
                          </h4>
                        </Link>
